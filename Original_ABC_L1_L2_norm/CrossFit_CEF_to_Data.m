...
%% some job settings
file_save_name = 'fminunc_original_1e-6_no_lnorms';
val = 1e-6;  % intial guess value
...
% lambda_L1 = 0; % Coefficient for L1 norm # Higher values increase sparsity in the parameter set.
% lambda_L2 = 0; % Coefficient for L2 norm # Higher values enforce small parameter magnitudes.
...
if T_ref ~= 0
    ...
    [guess_end,fval,exitflag,output1] = ...
...
[guess_end,end_error,exitflag,output2] = ...
    fminunc(@(guess_end)cross_obj_fun_sym(guess_end,guess_excess,X,Y,Z,eV,...
    x_exp,y_exp,dd,X0,Y0,x_vals,y_vals,-muhg_o,Temp,Gdft,dG_dy,d_chem,...
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    if T_ref ~= 0  
        % NOTE: I pass in -muhg_o because the fit is dG/dy = -mu
        [guess_excess,drop_error,exitflag,output3]  =...
...
% 
% [Gold,Hold,Sold] = plot_model(file_save_name + "_workspace",2);
% save(file_save_name + "_model",'Gold');
% 
% Gold_current = Gold;
% load('fminunc_original_1e-6_model');
% compare_models(Gold_current,Gold);
