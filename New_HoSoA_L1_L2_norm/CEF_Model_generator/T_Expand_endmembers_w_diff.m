function [G_sol,G_dft,Go,Go_dft] = T_Expand_endmembers_w_diff(G_soln,G_o,cp)
    ...
    syms gDiffSo [1 4] real
         ...
         %% Cp model    
     cp_exp = gDiffA + T*gDiffB + T^2*gDiffC + T^3*gDiffD + gDiffE/T^2;
     
     % H, S, and G
     dH = int(cp_exp,T) + gDiffHo;   % h is constant from integration
     dS = int(cp_exp/T,T) + gDiffSo; % s is constant from integration
     dG = expand(dH - T*dS);
    %% Full model subs 
